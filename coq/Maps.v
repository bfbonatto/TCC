...
Lemma not_none_implies_some : forall T (c : partial_map T) x,
  c x <> None -> exists v, c x = Some v.
Proof.
  intros. destruct (c x). exists t; auto. destruct H; auto. Qed.

Lemma update_not_empty : forall {T: Type} (m: partial_map T) x v,
  (x |-> v; m) <> empty.
Proof.
  intros. intro. pose (equal_f H). pose (e x).
  rewrite update_eq in e0; auto. rewrite apply_empty in e0.
  discriminate. Qed.

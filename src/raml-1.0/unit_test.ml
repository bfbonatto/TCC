...
let test_analysis degree (_, costs) _ exp_sln_stack =
  ...
  let analyze acc (metric, m_name) eval_cost = 
	...
	failwith "Linear program  is infeasable.\n"
      | Some (q, _) -> begin
	print_string ("-> okay\n")
	; print_string @@ "   metric = " ^ m_name ^ "; checking bound ...   "
	; flush stdout
	; if eval_cost -. q > Rconfig.float_max_error then
  ...
  let num_constr = List.fold2_exn ~init:0 met_names eval_costs ~f:analyze in
  print_string @@ "   Total number of constraints: " ^ (Int.to_string num_constr) ^ "\n"

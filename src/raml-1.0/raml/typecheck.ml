...
module T = struct
  type t = raml_type with sexp, compare
end
...
let typecheck ?linear:(linear=false) exp constr_map =
  ...
  let rec tcheck context exp =
	...
	match f with
	  ...
	  | Nat_minus
	  | Nat_div_mod -> check_fun t_exp [Ttuple [Tnat; Tnat]; Ttuple [Tnat; Tnat]]
	...
	; check_linear (List.map (e::es) (fun x -> (x,[])))
      ...
      | Elambda ((x,t_x),e) ->
	tcheck (Map.add context x t_x) e
...
let typecheck_stack ?linear:(linear=false) exp constr_map =
  ...
  let rec tcheck tstack_mode context exp = 
	  ...
	  match t_exp with
		...
		match targ_exp with
		  ...
		  | targ1::targs ->
			...
			Tstack_open
		    in
		    let e' = tcheck tstack_mode_e1 (Map.add context x t_x) e in
	...
	let e2' = tcheck tstack_mode (Map.add context x t) e2 in
	t_exp == (bounded_type e2')
	; List.iter2_exn e1'.exp_info e2'.exp_info (==)
	; check_linear [e,[] ; e1,[] ]
	; check_linear [e,[] ; e2,[x,t] ]

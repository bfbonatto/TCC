...
struct
  ...
  module Anno = Annotations.Make(Solver) 
  ...
  let zero_fanno_type rtype =
    let analyze targs tres rtanno metric =
      assert (rtanno.tan_type = tres);
      let () = Anno.zero_out_tanno rtanno in
      Anno.fresh_zero_tanno rtanno.tan_deg (Ttuple targs)
    in
    make_fanno_type analyze rtype
  ...
  let rec forward 
	...
	| Elambda ((x,t_x),e) -> 
	  begin
		...
		let fan_map fts =
		  match fts with
		    | ft_x::fts ->
		      let sigma' = Map.add sigma x ft_x in
		      let ft_ret = 
			match forward sigma' e with
			  | Fopen open_ftype -> 
			    open_ftype fts
	...
	| Eshare (e1,(x1,_),(x2,_),e2) -> 
	  ...
	  let sigma' = 	 
	    let sigma' = Map.add sigma x1 ft_x in
	    Map.add sigma' x2 ft_x
	  in
	  forward sigma' e2
	...
	| Enat_match (e,e1,(y,t_y),e2) ->
	  ...
	  let sigma' = Map.add sigma y Fnat in
  ...
  and backward 
	...
	in
	let backward_op = 
	  if app_const_pass_through then
	...
	match (arg.anl_exp).exp_desc with
	  ...
	  | Elet (x_opt, e1, e2) ->
		...
		| Some (x,t_x) ->
		  ...
		  let sigma' = Map.add arg_sigma x ft1 in
		  let gamma2 = 
		    backward 
		      {arg with 
			anl_sigma = sigma';
		  ...
		  begin
			...
			let anno_map = 
			  ...
			  let add_ind (is,ci) anno_map =
			    ...
			    if deg = 0 then
			      let ci_x = Map.add ci x ind_zero_x in
			      let q = can_find gamma2 (is,ci_x)  in
			      Map.add anno_map (is,ci) (Left q)
			    else
			      let tanno =
				let tan_type = t_x in
				let tan_map i = 
				  let ci' = Map.add ci x i in
				...
				backward
			      ...
			      in
			      Map.add anno_map (is,ci) (Right canno)
			  in
	  ...
	  | Eshare (e1,(x1,t1),(x2,t2),e2) -> 
	    ...
	    let sigma' =
	      ...
	      let sigma = Map.add sigma x1 ft_y in
	      Map.add sigma x2 ft_y
	    in
	    let gamma2 = backward
	      { arg with
		anl_sigma = sigma';
		...
		| ft::fts ->  (* keep arguments open *)
		  let gamma_body = backward
		    { arg with
		      anl_exp = e;
		      anl_tstack = fts;
		      anl_sigma = Map.add arg_sigma x ft;
		      anl_level = arg_level + 1
		    }
		  in
	  ...
	  | Eapp (name, e, es) ->
		...
		| Fopen open_ft ->
		  begin 
			...
			let gamma = Anno.combine_cannos_let gamma1 gamma2 in
			let cost = 
      			  (* This is a hack that seems to work for most metrics.
			     Becouse we remove the arg_tstack from the type stack
			     when analyzing e, the last lambda accounts for the
			     cost of the closure that is created for expression e.
			     However, this closure is not created because of a
			     non-empty stack.  So we remove the cost here.
			     This is problematic since we don't know the size of
			     the closure at this point.  However, it works for
			     eval steps.  The right thing to do would be to add
			     a counter to the analysis argument to keep track of
			     the size of the 'real' stack.
			  *)
			  let c1 = metric (Mapp (List.length es)) in
			  c1 -. (metric (Mclosure (0)))
			in
			Anno.add_cost gamma cost
		  end
	  ...
	  | Eundefined ->
	    ...
	    Anno.fresh_canno arg_deg tstack String.Map.empty
  ...
  let analyze_expression ~degree ~metric ?(collect_fun_types = Pnone) exp =
    let arg =
      ...
      ; anl_metric = metric
      ; anl_deg = degree
      ; anl_rtanno = Anno.fresh_tanno degree exp.exp_type
      ; anl_sigma = String.Map.empty
      ; anl_tstack = []
      ; anl_level = 0
      }
    in
    init collect_fun_types;
  ...
  let analyze_function ~degree ~metric exp =
	...
	begin
	  match qss with
		...
		try
		  ...
		  in
		  Some (update_tanno atarg, update_tanno atres)
		with LP_infeasable ->
		  None

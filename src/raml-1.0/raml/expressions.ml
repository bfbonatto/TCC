...
type 'a var_rbind = var_id * 'a rtype with sexp
type var_bind = unit var_rbind with sexp
...
let string_of_builtin_fun f =
  ...
  match f with
    ...
    | Nat_of_intc n -> nat_module ("of_int " ^ string_of_int(n))

...
struct
  ...
  let solution = ref [| |]
  let _ = Clp.set_log_level !clp_state Options.log_level (* use n>0 for debugging *)
  ...
  let add_objective v q =
    objective := Map.add !objective v q
  ...
  let copy_objective () =
    ...
    Map.iter !objective (fun ~key ~data -> Array.set arr key data)

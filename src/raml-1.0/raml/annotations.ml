...
struct
  ...
  let fresh_tanno ?qzero deg t = 
	...
	| Some q ->
	  fun ind acc ->
	    ...
	    Mi.add acc ind var
	| None ->
	  fun ind acc -> Mi.add acc ind (Solver.fresh_var ())
    in
  ...
  let fresh_canno ?qzero deg ts context =
	...
	| Some q ->
	  fun cind acc ->
	    ...
	    Mc.add acc cind var
	| None ->
	  fun cind acc -> Mc.add acc cind (Solver.fresh_var ())
    in
  ...
  let geq_constr p q  = 
    Solver.add_constr_list ~lower:0.0 [(p,1.0); (q,-.1.0)]

  let geq_sum_constr ps qs =
  ...
  let stack_variable canno x t_x = 
	...
	| Some _ ->
	  let can_map (inds,ci) = 
	    match inds with
	      | i_x::is -> can_find canno (is, Map.add ci x i_x)
	      | _ -> raise (Anno_exn "Dead code.")
	  in
	  (Map.remove canno.can_context x, can_map)

	| None ->
	  let fresh_map = make_memo_cmap () in
	  let can_map (inds,ci) = 
 	    match inds with
	      | i_x::is -> 
  ...
  let combine_tannos tan1 tan2 =
    ...
    let add_ind ind () =
      geq_constr (tan_find tan ind) (tan_find tan1 ind)
      ; geq_constr (tan_find tan ind) (tan_find tan2 ind)
    in      
  ...
  let add_const backward = 
	...
	{ atanno with
      ...
      let _ = geq_sum_constr [p_new] [p_zero;c] in
      let _ = geq_sum_constr [q_new;c] [q_zero] in
      atanno'
  ...
  let nat_shift_t rtanno =
	...
	| Inat n ->
	  ...
	  begin 
	    match qs with
	      | [q] ->
		Map.add cmap p_ind q
	      | _ ->
		let p = Solver.fresh_var () in
		let () = geq_sum_constr [p] qs in
		Map.add cmap p_ind p
	  end
	| _ -> raise (Anno_exn "Dead code.")
    in 
  ...
  let nat_shift x_m canno x =
	...
	match Map.find ci x_m with
	  | Some (Inat n) -> 
	    ...
	    let q = can_find canno (is,ci) in
	    let ci1 = Map.add ci' x (Inat n) in
	    if max_deg then
	      let () = assert (not (Map.mem cmap (is,ci1))) in
	      Map.add cmap (is,ci1) q
	    else
	      let ci2 = Map.add ci' x (Inat (n+1)) in 
	      let p2 = Map.find_exn cmap (is,ci2) in
	      let p1 = Solver.fresh_var () in
	      let () = eq_sum_constr [p1;p2] [q] in
	      assert (not (Map.mem cmap (is,ci1)));
	      Map.add cmap (is,ci1) p1
	  | None -> 
	    let q = can_find canno (is,ci) in
	    let ci1 = Map.add ci x (Inat 0) in
	    if max_deg then
	      let () = assert (not (Map.mem cmap (is,ci1))) in
	      Map.add cmap (is,ci1) q
            else
	      let ci2 = Map.add ci x (Inat 1) in 
	      let p2 = Solver.fresh_var () in
	      let p1 = Solver.fresh_var () in
	      let () = eq_sum_constr [p1;p2] [q] in
	      assert (not (Map.mem cmap (is,ci1)));
	      assert (not (Map.mem cmap (is,ci2)));
	      let cmap = Map.add cmap (is,ci1) p1 in
	      Map.add cmap (is,ci2) p2
...
(* XXX There's bug a in the following code. 
  ...
  fun yts canno (cid,t_ind) x ->
	...
	let shift_cont is ci ci' j1 js (cs,cmap) =
  	  let q = can_find canno (is,ci) in
	  let ci1 = Map.add ci' x (Iind ((j1,cid)::js)) in
	  let cinds = 
	    ...
	    if is_zero_index j1 then
	      let ci2 = Map.add ci' x (Iind js) in 
...
*************************** *)
  ...
  let fold_tuple y canno xts = 
    ...
    assert ((List.mem xs y) || (not (Map.mem canno.can_context y)));
	...
	match Map.find cont x with
      ...
      (Map.add cont y (Ttuple ts), missing)
    in
    let new_cmap ind =
      let memo_cmap = make_memo_cmap () in
  ...
  let share y (x1,x2) canno =
	...
	| t when is_const_type t ->
	  begin
		...
		let ci' = Map.remove ci y in
		let ci' = Map.add ci' x1 zero_ind in
		Map.add ci' x2 zero_ind
	      in
	      can_find canno (is,ci')
	  end
	| Tnat
  ...
  let update_context canno xts context =
	...
	if Map.mem canno.can_context y then
	  ...
	  Map.add zero_inds y (zero_index t_y)
      in

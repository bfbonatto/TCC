...
module C = struct
  type t = cindex with sexp, compare
  let hash = Hashtbl.hash
end
...
let tracked_inductive_index =
  let valid t (i,cid) = 
    (constr_degree cid t > 0) || (degree i > 0)
  in
  let multi_cons t = 
    match t with
      | Tind clist ->
  ...
  fun t ics ->
    match ics with
      | [] -> true
      | [i,cid] ->
	(degree i > 0) || (multi_cons t) || (constr_degree cid t > 0)
      | _  -> List.for_all ics (valid t)
...
let prioritized_indices t deg =
  ...
  let cmp (i1,d1,l1) (i2,d2,l2) =
    ...
    if diff = 0 then
      ...
      diff
  in
  let inds = List.sort ~cmp inds in

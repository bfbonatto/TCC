...
let evaluate : 
  ...
  = fun exp metrics ->
    ...
    let store value =
      let l = fresh_loc () in
      heap := Map.add !heap ~key:l ~data:value;
      l
    in
            ...
            | Vtuple [l1; l2] -> begin
              match lookup l1, lookup l2 with
                | Vnat n1, Vnat n2 ->
                  ...
                  count (Mbfun_eval f);
                  store (Vtuple (List.map [Vnat ndiv; Vnat nmod] store))
                | _ -> raise wrong_arg_type
              end
            | _ -> raise wrong_arg_type
        end
              ...
              match lookup l1, lookup l2 with
                | Varray (arr,n), Vnat i ->
                  ...
                  if i < 0 || i > n-1 then
                    ...
                    heap := Map.add !heap l1 (Varray (Map.add arr i l3, n));
                  store (Vbc Cunit)
                | _ -> raise wrong_arg_type
              end
            | _ -> raise wrong_arg_type
	...
	if l > n then
      ...
      let interpret_loc loc = match lookup loc, args with
        ...
        | Vclosure ( x, e, stack_cl), l1::ls ->
          let stack' = Map.add stack_cl x l1 in
	...
	| Eapp (_,e,es) ->
	  let cont' results = 
	    ...
	    eval e (ls@args) stack 
	      (fun l -> count (Mapp (List.length es)); cont l)
	  in
	...
	| Elambda ((x,t),e) -> (
	  match args with
		...
		~f:(fun ~key:x ~data:_ -> 
	      ...
	      cont l
	    | l::ls ->
	      let stack' = Map.add stack x l in
	      eval e ls stack' cont
	)
	...
	| Elet (x_opt,e1,e2) ->
	  ...
	  let cont' l1 = 
	    let stack' =
	      match x_opt with
		| None -> stack
		| Some (x,t) -> Map.add stack ~key:x ~data:l1
	    in
	...
	| Eshare (e1,(x1,t1),(x2,t2),e2) ->
	  let cont' l1 = 
	    count Mshare; 
	    let stack' = Map.add (Map.add stack x1 l1) x2 l1 in
	    eval e2 args stack' (fun l -> cont l)
	  in
	  eval e1 [] stack cont'
	...
	| Enat_match (e,e1,x,e2) -> 
	  let cont' l = 
		...
		if n = 0 then
		  ...
		  let stack' = Map.add stack (fst x) l in
	...
	| Etuple es ->
	  check_args 0;
	  eval_all (List.rev es) stack (fun ls -> 
 	    count (Mtuple (List.length es))
	    ; cont (store (Vtuple (List.rev ls) ))
	  )
